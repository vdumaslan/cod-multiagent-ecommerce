...
This note documents why the current systemâ€™s outputs often feel generic / impractical for a small-business owner, based on direct inspection of:
...
- The inventory cache produced by `src/scripts/precompute/build_inventory_cache.py` can become degenerate (nearly all items in the same class) if the rule thresholds do not match the data distribution.
...
**Where this happens**
- Cache build rules: `copilot-v2/src/scripts/precompute/build_inventory_cache.py`
- Runtime consumption: `copilot-v2/app/agents/inventory_agent.py` + enrichment in `copilot-v2/app/pipeline.py`

**Solutions**
- Before assuming the entire cache is â€œbrokenâ€, verify whether the **`stockout_risk`** rule can ever fire (cases where `on_hand_units <= safety_stock_units`). If that count is near-zero too, the current ruleset is effectively unusable for decision-making.
...
- Add a QA check in precompute: if distribution is >95% one class, mark cache â€œunreliableâ€ and/or auto-tune thresholds.

---

### 2) Pricing cache is heavily saturated at the policy bounds (Â±10%)

- The pricing cache (`predicted_price_change_pct`) is concentrated near Â±10%, meaning the system frequently recommends extreme repricing.
- This pushes the Advocate to repeat â€œ+10%â€ / â€œ-10%â€ behavior because it is literally being fed those deltas as the primary â€œactionâ€.
...
**Solutions**
- Avoid linear shrinkage alone (it can just move the pileup from Â±10% to a new â€œeffective boundâ€).
...
- Include a â€œnear_bound=true/falseâ€ flag in the LLM payload so debate agents treat bound-hit outputs skeptically.
- Add richer business signals so pricing isnâ€™t the only lever (see â€œpayload contractsâ€ section).
...
- review recency / â€œwhat changed recentlyâ€
- explicit objective function (profit vs revenue vs liquidation vs reduce returns)
- operational constraints (max # SKUs, donâ€™t raise price when \(p_{neg}\) high, etc.)
...
**Solutions**
- Pass structured, decision-ready features from `products.parquet` and `product_signals.parquet` to debate agents (see â€œWhat to passâ€ section).
...
- Example run: `copilot-v2/artifacts/runs/38710839ca6e1009/run_20260426_061542_store_00_judge/8_debate_judge.json`
  - The judgeâ€™s first response was non-JSON (narrative) and failed parsing (`judge_parse_error`), requiring a retry that produced valid JSON.
...
### 5) UI scoring/labels can make outputs feel â€œmade upâ€

In `copilot-v2/app/ui/src/App.jsx`:

- â€œConfidenceâ€ is derived mainly from `retrieval_score * 100` (clamped), not a calibrated confidence.
- â€œImpact scoreâ€ is a transform of confidence.
- â€œRiskâ€ is driven largely by sentiment because inventory is degenerate.

Even if backend improves, presenting these as â€œImpact/Confidenceâ€ can erode trust.
...
**Solutions**
- Rename confidence to what it is (e.g., â€œRetrieval similarityâ€).
- Compute risk from meaningful, validated signals (inventory + returns + sentiment) once inventory is fixed.
- Add an â€œevidence panelâ€ (why selected, what signals drove the decision).
- Surface the **retrieval snippet** that already exists at `evidence.points[0].text` (computed in `copilot-v2/app/pipeline.py` but not shown in the UI). This is the easiest â€œwhy this SKU?â€ improvement.
- Guard the â€œmock plansâ€ fallback: the UI currently falls back to hardcoded mock plans if the real results array is empty. Those mock plans can mask pipeline failures during demos. Prefer an explicit â€œNo results / API degradedâ€ state over showing mock data without warning.

---

## What your actual dataset says (and whatâ€™s hurting output quality)
...
- Some `product_document` rows are placeholders like â€œPRODUCT NOT AVAILABLEâ€, which can pollute candidate sets for broad queries.
- Retrieval score separation for generic goals can be weak (scores cluster tightly), so downstream agents see â€œmany similar candidatesâ€.
...
- Example run: `copilot-v2/artifacts/runs/38710839ca6e1009/run_20260426_064309_store_00/1_retrieval.json`
  - Top-10 retrieval scores ranged from **0.75966 â†’ 0.76749** (spread **0.00783**), indicating weak separation for broad goals.
...
- The â€œmainâ€ snapshot manifest `copilot-v2/artifacts/data_snapshots/38710839ca6e1009/manifest.json` may not reflect the enriched/in-place derived columns.
...
**Problem**
- These structured columns exist and are decision-relevant, but are not carried through the runtime enrichment â†’ debate payload in a structured way.
...
**Observations**
- `n_reviews` is relatively narrow (often near 15â€“24), which makes â€œconfidenceâ€ from sentiment alone questionable.
...
- Pass â€œmissingnessâ€ signals: if `n_reviews` is low, treat sentiment as low-confidence.
...
**Observed**
- Large fraction of predictions are near Â±10% (policy bound), making recommendations feel unrealistic.

**Solutions**
- Replace hard clipping with a smooth soft-cap transform (e.g., tanh) to avoid boundary pileups.
- Add a calibration layer and include confidence/uncertainty or â€œnear_boundâ€ flags.
...
**Goal**
- Find relevant candidate products for the userâ€™s goal with good separation and controllable filters.
...
- Avoid placeholder docs (e.g., â€œPRODUCT NOT AVAILABLEâ€).
...
- However, the runtime pipeline currently hardcodes `action_type="reprice"` for every enriched candidate (see `_enrich()` in `copilot-v2/app/pipeline.py`), and the Advocateâ€™s few-shot example only demonstrates `reprice`. Together, these strongly bias the debate toward repricing-only outputs.
- Recommended fix: add a lightweight playbook field during enrichment (e.g., `suggested_action`) derived from signals and pass it to Advocate/Critic/Judge. This is effectively blocked until inventory classification is non-degenerate (otherwise rules like low_stock â†’ hold never trigger).
...
5) Update UI to present metrics honestly (rename â€œconfidenceâ€, add evidence, avoid fake-looking scores).

**Additional fixes (no cache rebuild required)**
- F) **Retrieval minimum-score threshold**: filter out candidates below a configured minimum similarity score. If no candidates remain, return â€œno strong matchesâ€ instead of running debate on weak matches.
...
- **A â€” Judge payload JSON serialization**
...
- **F â€” Retrieval minimum-score threshold**
  ...
  - Observed: with `COPILOT_RETRIEVAL_MIN_SCORE=0.80`, broad goals (e.g., â€œhow to increase revenueâ€) can legitimately return **0** candidates (preferred vs debating weak matches).

- **G â€” Query reformulation before retrieval**
  - Status: implemented in `copilot-v2/app/pipeline.py` (writes `0_query_rewrite.json` per run and exposes `trace.query_rewrite`).
  - Observed:
    - For vague goals (e.g., â€œhow to increase revenueâ€), the rewriter returns a **clarifying question** and an empty `retrieval_query`, allowing retrieval to correctly return â€œno strong matchesâ€.
    - For semi-specific goals (e.g., â€œincrease revenue for bed risersâ€), the rewriter produces a concrete `retrieval_query` and retrieval scores can clear the min-score threshold (e.g. top score ~0.85).

- **B â€” Inventory thresholds (dataset-aware)**
  - Status: implemented in `copilot-v2/src/scripts/precompute/build_inventory_cache.py` and inventory cache rebuilt.
  - Observed:
...
- **C â€” Pricing soft-cap (tanh)**
  - Status: implemented in `copilot-v2/src/scripts/precompute/build_pricing_cache.py` and pricing cache rebuilt.
  - Observed:
    - Previously, >80% of predictions were within \(|pct|\ge 9.5\%\) (hard-bound pileup).
    - After soft-cap, predictions no longer pile up at Â±10% and the observed range compressed smoothly (example: min â‰ˆ -7.76, max â‰ˆ 7.69 on the rebuilt cache).
    - Remaining issue: predictions are still somewhat â€œpeakyâ€ (e.g., many items clustering around ~7â€“8%). This is now a model/calibration characteristic (not a clipping artifact). Track it as a follow-up:
      - add and pass a `near_bound`-style flag for â€œlarge magnitudeâ€ deltas (relative to policy bound), and/or
      - add a calibration/shrinkage layer based on additional evidence signals.

- **D â€” Action space / playbook wiring**
  ...
  - Observed (spot-check run):
    - Pipeline produced non-trivial `suggested_action` distribution (example run: 9Ã— `reprice`, 1Ã— `hold`).
  ...
  - Notes:
    - This is intentionally â€œlightweightâ€ (heuristics), but it breaks the previous degeneracy where the system could only ever recommend `reprice`.

- **E â€” UI: evidence-first rendering + honest labels**
  - Status: implemented (UI only).
  - What changed:
    - `copilot-v2/app/ui/src/App.jsx`: carries through `suggested_action`, `finalActionType`, `evidenceSnippet`, and `retrievalSimilarity` from `ranked_actions` into the UI â€œplanâ€ model.
    - `copilot-v2/app/ui/src/components/ResultsView.jsx`: displays (a) suggested vs final action, (b) retrieval snippet, and relabels â€œConfidenceâ€ â†’ â€œRetrieval similarityâ€.
    - `copilot-v2/app/api/schemas.py`: adds `suggested_action` to `RankedAction` response model so the UI reliably receives it.
  - Observed:
    - `ranked_actions[*].suggested_action` now appears in `/debate/judge` responses and renders in the Results UI alongside the Judgeâ€™s final `action_type`.

- **E2 â€” Inventory-safety guardrail (Judge alignment)**
  - Status: implemented (backend).
  - Why: after Step D/E, we could see cases where `suggested_action=hold` for `low_stock` items but the Judge still returned `reprice` (often `0%`), which conflicts with the business goal â€œavoid stockoutsâ€.
  - What changed:
  ...
  - Verified:
    - For goal â€œAvoid stockoutsâ€¦â€ the `/debate/judge` output now returns `final_action=hold` for low-stock candidates (no fallback).
...
- A) Judge payload JSON bug â†’ quick reliability win
- F) Retrieval score minimum threshold â†’ honest â€œno matchâ€ behavior
- G) Query reformulation â†’ improves candidate quality before debate
- B) Inventory thresholds â†’ rebuild cache â†’ unlock meaningful risk/action playbooks
- C) Pricing soft-cap (tanh) â†’ rebuild cache â†’ reduce extreme â€œÂ±10% everywhereâ€ behavior
...
- Objective selector (profit / revenue / clear inventory / reduce returns / avoid stockouts): implemented end-to-end (UI â†’ schema â†’ prompts).
- Constraint filters: implemented end-to-end (UI â†’ schema â†’ pipeline enforcement)
...
If a gate fails, the system should either (a) refuse to start, or (b) run in â€œdegraded modeâ€ with clear UI messaging.

### Retrieval QA

- **Corpus hygiene**
  - Flag or remove documents whose `product_document` contains placeholder strings like â€œPRODUCT NOT AVAILABLEâ€.
...
- **Coverage**
  - Require coverage threshold (e.g., â‰¥90% of products in snapshot) or explicitly mark missing products as â€œno pricing modelâ€ and avoid repricing recommendations for them.
...
- **Soft-cap sanity (if using tanh soft-cap)**
  - Gate: after applying \(b\cdot\tanh(x/b)\), check that extreme pileups are reduced:
    - \(|pct| \ge 0.95b\) should be â€œrareâ€ (set a target like <10â€“15%).
...
- **Explainability**
  - Add at least a lightweight explanation artifact (e.g., top negative keywords per product or per category) so the UI can justify â€œwhy risk is highâ€.
...
- **Fallback rate**
  ...
  - Gate: if judge fallback is used at all (or exceeds, say, 1â€“2% over a replay set), treat the judge prompt/payload as failing QA.
- **Critic output robustness**
  - Even when parsing succeeds, watch for Critic responses that include stray trailing text after the JSON object (the JSON extractor may â€œsucceed silentlyâ€ by pulling the first balanced object).
...
- **Metric honesty**
  - If â€œconfidenceâ€ is derived from retrieval similarity, label it accordingly.
- **Evidence-first rendering**
  - For each ranked plan, show the top 1â€“2 signals that drove it (price position, sentiment risk, inventory risk) and indicate missing data explicitly.
- **No silent mock data**
...
- **Show retrieval snippet**
  - Gate: each displayed plan should show a short â€œwhy this SKUâ€ excerpt from `evidence.points[0].text` when available.

---

## Remaining issues / things to pay attention to (next priorities)

These are the main reasons outputs can still feel â€œnot reasonableâ€ even after the fixes above, plus the most practical next improvements.

### 1) Business grounding is still thin (data limits)

- **Missing hard blockers**: we still donâ€™t have true decision drivers like:
...
### 2) Pricing calibration is still â€œpeakyâ€ (post soft-cap)

- Step C removed the Â±policy-bound pileups, but **predictions still cluster** (e.g., ~7â€“8%).
...
- The UI now previews this, but there are two product decisions to consider:
  - **Strict**: keep â€œ0 matchesâ€ behavior (honest), and push users to scope to a category/subcategory.
  - **Soft fallback**: show â€œweak matchesâ€ below threshold (clearly labeled), to help users iterate faster.

### 4) Catalog document quality can still be low-signal

- Some `product_document` entries are extremely short/placeholder-like (e.g., â€œfilterâ€).
...
- Even when actions are guarded (e.g., low-stock â†’ `hold`), the **rationale bullets** can still be generic.
- Next most impactful improvement for â€œsmall business usefulnessâ€:
  - enforce a stricter rationale schema (must cite specific provided fields), and/or
  - compute deterministic â€œtop driver signalsâ€ in code and render them in the UI (so explanations are auditable even if the LLM is vague).

### 6) Objective semantics (especially â€œprofitâ€) are limited

- Because margin/COGS are missing, â€œprofitâ€ can only be used as a **policy framing** (e.g., be conservative on discounting), not a true optimizer.
- If profit is a core demo goal, youâ€™ll need a cost proxy or add synthetic COGS.
...
- If you want â€œdo this tomorrowâ€ outputs, youâ€™ll need either downstream integrations or a more concrete playbook with parameters.
...
- For a production system or different product categories, these thresholds may need tuning â€” currently that requires a code change.
- **Note**: the shrinkage clamping bounds (0.70, 0.60, 0.80) are also hardcoded but intentionally so â€” they act as safety guardrails and should not be made configurable.


---

## Post-implementation fixes (after Aâ€“G + UI redesign)

These issues were identified by inspecting the two live runs after all Aâ€“G fixes were applied (snapshot `38710839ca6e1009`, queries: "Incxrease revenue for bed risers" and "Increase revenue for bed risers").

### Completed

- **mockPlans removed** â€” `mockPlans` constant in `copilot-v2/app/ui/src/App.jsx` was dead code (never used to populate the UI after E was implemented). Removed.
- **Impact Score removed** â€” `impactScore` was `retrievalSimilarity * 0.92`, a meaningless transform of retrieval score. Removed from `buildPlansFromRanked` in `App.jsx` and from the results card in `ResultsView.jsx`. The results card now shows only Risk Level and Retrieval similarity.

### Still outstanding

**Bugs (actively wrong)**

1. **Rationale hallucination** *(see Remaining issues #5)* â€” **Completed.** Implemented deterministic, signal-grounded rationale bullets in `Pipeline._merge_judge_output()` and auto-replaced empty/generic judge rationales with these bullets (pricing availability/flags, inventory, sentiment, returns, retrieval similarity).

2. **Typo handling in query rewriter** *(new finding from live runs)* â€” **Completed.** Added lightweight typo correction for common intent words (including adjacent transpositions like `Gorw â†’ Grow`) and surfaced a `notes` field in `trace.query_rewrite` when corrections are applied.

3. **Advocate pricing direction** *(related to Remaining issues #2)* â€” **Completed.** Updated the Advocate prompt to start from the cache delta (`recommended_price_change_pct`) and treat `pricing_source="fallback"` as `investigate` with `0.0%` (no invented reprices).

**Quality gaps**

4. **Debate is not genuinely adversarial** *(related to Remaining issues #8)* â€” **Completed.** Made debate rounds causally linked: Critic disagreements are forced to cite `product_id` + concrete signals; Advocate revision is enforced (auto-rerun if unchanged); Judge output is deterministically linked to the latest Advocate plan (default per-SKU + default SKU selection).

5. **`suggested_action` is always "reprice"** *(related to Remaining issues #1)* â€” **Completed.** Tuned the playbook in `pipeline.py` so `suggested_action` fires on moderate signals (returns/sentiment), objective, pricing availability (fallback â†’ investigate), and inventory risk (hold/restock), producing more diverse actions.

6. **Pricing still clusters around 7â€“8%** *(see Remaining issues #2)* â€” **Completed.** Added `pricing.large_delta` / `pricing.near_bound` flags to enriched candidates and passed them through debate payloads + UI Top Drivers. Added optional, tunable shrinkage (`COPILOT_ENABLE_PRICING_SHRINKAGE=1`) with env-var controls for shrink factors.

7. **No price-missing indicator in UI** *(related to Remaining issues #1)* â€” **Completed.** Added `pricing.price_missing` in the backend (true when pricing cache is missing) and rendered a clear UI warning + Top Driver entry (â€œPrice: unknown / pricing unavailableâ€).

8. **Plan title is not user-friendly** *(new UI finding)* â€” **Completed.** Updated Results card titles to use product title parsed from the retrieval snippet (`evidence.points[0].text`) instead of raw `product_id`.

**Known limitations (require new data)**

9. **No COGS/margin** *(see Remaining issues #1 and #6)* â€” "Profit" objective is a proxy (conservative repricing framing). Already labelled as such in the UI (`"Grow profit (proxy)"`). Cannot be truly fixed without cost data.

10. **Operational action details missing** *(see Remaining issues #7)* â€” `restock`/`promote` actions have no quantities, channels, or durations attached. Would need a more concrete playbook with parameters or downstream integrations to produce "do this tomorrow" outputs.

### Suggested priority order

All items #1â€“#8 above have been completed; remaining work is primarily **known limitations** (#9â€“#10) and the broader â€œRemaining issues / next prioritiesâ€ section (data and operational robustness).
...
Three rounds of runs were performed after all Aâ€“G fixes were applied (snapshot `38710839ca6e1009`). Each round validated a new set of fixes.

---

### Round 1 â€” Initial validation runs

**Setup**: query â€œReduce returns for bed risersâ€, objective: reduce returns, `do_not_raise_if_p_neg_above=0.3`.

**What was working**
- `price_missing=true` correctly flagged for products with fallback pricing source
- `large_delta=true` correctly firing when delta â‰¥ ~7%
...
- Retrieval min-score threshold (`COPILOT_RETRIEVAL_MIN_SCORE=0.80`) not enforced when env var was absent â€” retrieval agent defaults to `0.0`, so ~0.76-scoring non-bed-riser products passed through

---

### Round 2 â€” Post-fix validation (A, B, C, E)
...
**What was working**
- Fix B confirmed: match preview correctly showed â€œNeeds clarificationâ€ and did not proceed with retrieval when rewriter returned both fields
...
- Retrieval scores were ~0.76 (below threshold) because `COPILOT_RETRIEVAL_MIN_SCORE` was not set in the environment for that run â€” confirmed the default was still `0.0` at the time

---

### Fixes applied (from live run findings)

- **A â€” Rationale hallucination**: Two-layer fix:
  - *Layer 1 (LLM)* â€” Added rationale grounding rule to `_SYSTEM` in `judge.py` and updated all three agents' few-shot examples to model correct numeric citations.
  - *Layer 2 (deterministic override)* â€” LLM grounding alone was still hallucinating specific numbers. Changed `_merge_judge_output()` in `pipeline.py` to always replace rationale bullets with deterministic ones built from the actual input data. This follows the same principle as RAG: the LLM's *reasoning* (what action to take) stays LLM-driven; the *factual citations* (field values from the payload) are grounded deterministically. The `_looks_generic()` check is no longer the gate â€” grounding is always enforced.
- **B â€” Query rewrite either/or**: Added `if rq and cq: rq = â€œâ€` in `pipeline.py` â€” if the rewriter returns both a clarifying question and a retrieval query, treat as clarification and do not proceed with retrieval.
- **C â€” Judge large_delta + hold guardrail**: Added `if suggested_ld == â€œholdâ€ and large_delta: continue` to the debate linkage skip condition in `judge.py` â€” prevents the linkage loop from overwriting a hold that the guardrail already set.
- **D â€” Nondeterministic query rewriter**: No action needed (expected LLM behavior; both rewrite paths handled correctly after B).
- **E â€” Retrieval min-score not defaulting correctly**: Fixed by hardcoding `0.80` as the default in `pipeline.py` so the threshold is always active without requiring the env var.

---

### Round 3 â€” Final validation run

**Setup**: query â€œReduce returns for bed risers due to negative customer reviews and quality complaintsâ€, objective: reduce returns, `do_not_raise_if_p_neg_above=0.3`. All fixes (A layer 2, E) applied.
...
|---|---|---|---|---|---|---|
| 1 | B09QM99LKG (Banqin Bed Risers) | reprice +5.33% | 0.08 | 2 | false | âœ“ |
| 2 | B0C6FRCQDS (Headwind Furniture Riser) | hold | 0.00 | 4 | false | âœ“ |
| 3 | B07D4358F2 (Tech Team Bed Risers) | investigate | 0.29 | 3 | true | âœ“ |

**All fixes confirmed**
- Retrieval scores 0.83 / 0.83 / 0.82 â€” all above 0.80 threshold, all actual bed risers (E âœ“)
- Rationale bullets exactly match real field values â€” no hallucinated numbers (A âœ“)
- Rank 2 correctly `hold` due to `low_stock + risk_flag=true` guardrail (inventory guardrail âœ“)
- Rank 3 correctly `investigate` due to `p_neg=0.29` + `large_delta=true` (C âœ“)
- Actions are sensible for â€œreduce returnsâ€ objective: no unjustified upward repricing on high-return items

---

## RL (Contextual Bandit) for Version B

This repo includes a lightweight **contextual bandit** that tunes internal â€œknobsâ€ for **Version B (AI Copilot)** to optimize for:
...
RL selects one â€œarmâ€ per run and applies its configuration for that run only:
...
- **Run start (Version B)**: `copilot-v2/app/pipeline.py`
  ...
  - attaches `run_id â†’ arm_id` (so later events can be credited)
...
- `-2.0` if `abandoned`
- confidence rating (1â€“5) adds \(0.6 \times (c - 3)\)
...
- a small `run_id â†’ arm_id` map to credit events correctly
