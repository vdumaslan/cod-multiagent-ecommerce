...
| Pricing | TabPFN | `n_estimators=4`, `softmax_temperature=0.85`, `max_fit_rows=21119`, `policy_bound=Â±10%` |
...
**Why `tabular_features.parquet` for pricing:** This is almost certainly the same table TabPFN was trained on â€” it has a `split` column, the pricing manifest references a training table derived from it, and the feature columns match. Using it for inference guarantees the feature schema is identical to what the model saw during training. Coverage is limited to ~50k products as a known trade-off.

**Files not used at runtime:**
- `priced_subset/` variants â€” training/eval subsets only
- `reviews_sentiment_balanced.parquet` â€” balanced training split used during model training only
- `product_signals.parquet`, `products.parquet` â€” covered by `tabular_features.parquet` for pricing; not needed separately
...
- `RetrievalAgent` class with three methods:
  - `load_index()` â€” loads the pre-built FAISS index from `artifacts/indexes/{snapshot_id}/dense/intfloat_e5-large-v2/` (fast, seconds). Returns `True` if successful. **This is the normal startup path.**
  - `build_index(corpus, save=True)` â€” encodes the full corpus and builds a new FAISS index. Saves to disk automatically. Only needed when the product catalog or model changes.
  - `retrieve(query)` â€” encodes the user query (fast, single text) and searches the index for top-k results
...
- Runs inference on all ~50k rows, clips predictions to `Â±10%`
...
- Joins on `product_id` and applies the same 4-class rule thresholds as `src/runtime/inventory_agent.py`
...
- Loads `pricing_cache.parquet` once at startup into a `dict[product_id â†’ predicted_price_change_pct]`
- Exposes `lookup(product_id)` and `lookup_many(product_ids)` â€” pure dictionary reads at runtime

### `agents/sentiment_agent.py`
- Loads `sentiment_cache.parquet` once at startup into a `dict[product_id â†’ {n_reviews, p_pos, p_neu, p_neg}]`
- Exposes `lookup(product_id)` and `lookup_many(product_ids)`

### `agents/inventory_agent.py`
- Loads `inventory_cache.parquet` once at startup into a `dict[product_id â†’ {stock_status, risk_flag, ...}]`
- Exposes `lookup(product_id)` and `lookup_many(product_ids)`
...
All precompute scripts live in `src/scripts/precompute/`. This is the single location for all offline cache-building steps â€” pricing, sentiment, inventory, and retrieval indexes.
...
The `app/precompute/` directory has been removed â€” `src/scripts/precompute/` is now the single source of truth.
...
### `llm.py`
Lightweight self-contained LLM utility â€” no third-party SDK.
- `OllamaClient` â€” posts to the Ollama `/api/chat` endpoint
- `OllamaChatResult` â€” typed result with `content` and `elapsed_s`
- `extract_json_object()` â€” extracts the first valid JSON object from a raw model response
- Schema validators: `validate_ranked_actions()`, `validate_specialist_proposal()`, `validate_peer_review()`
- `SchemaError` â€” raised when a parsed object fails schema validation

### `agents/orchestrator/advocate.py`
- `build_messages()` â€” builds the Ollama chat messages with `is_revision` flag
  ...
  - Payload is slimmed to only the fields the LLM needs (product_id, pricing source, sentiment scores, inventory status, signals) and serialized as proper JSON via `json.dumps()` â€” not Python dict syntax
- `run()` â€” calls Ollama, retries twice on JSON parse failure, falls back to top-k candidates by retrieval order
...
- `build_messages()` â€” receives the advocate proposal and challenges it; payload slimmed to goal, constraints, advocate output, and candidate inventory/availability signals; serialized with `json.dumps()`
- Output schema: `{agreements: [...], disagreements: [...], suggested_changes: [...]}`
- `run()` â€” calls Ollama, retries once on failure, falls back to empty peer review

### `agents/orchestrator/judge.py`
- `build_messages()` â€” receives final advocate + critic outputs plus any human feedback
- `_fallback_from_baseline()` â€” returns the baseline ranked actions if JSON fails
- `run()` â€” returns `(final_actions, raw_trace, judge_fallback: bool)`
...
Three public functions â€” no judge runs except in `run_judge_only()`:

```
run_acj()
  Round 1: Advocate â†’ Critic
...
continue_acj(prev_advocate, prev_critic, human_feedback?)
  Revision round: Advocate (sees prev critic as CRITIC_FEEDBACK) â†’ Critic
...
**`run_pipeline()`** â€” called by `POST /pipeline`:
1. `RetrievalAgent.retrieve(goal)` â†’ candidate list; writes `1_retrieval.json`
2. `_enrich()` â†’ attaches pricing, sentiment, inventory signals; writes `2_enriched.json`
3. `_make_baseline()` â†’ sorts by retrieval score, `price_change=0`; writes `3_baseline.json`
4. Returns `enriched_candidates` + `baseline_ranked_actions` â€” no LLM calls

**`start_debate()`** â€” called by `POST /debate/start`:
- Calls `run_acj()` with pre-enriched candidates (no retrieval re-run)
- Creates `run_{ts}_{owner}_debate/` folder; writes stages 4â€“5
- Returns `{ok, advocate, critic, debate_trace}`

**`continue_debate()`** â€” called by `POST /debate/continue`:
- Calls `continue_acj(prev_advocate, prev_critic, human_feedback?)`
- Creates a separate `run_{ts}_{owner}_cont/` folder
- Returns `{ok, advocate, critic, raw}` â€” no ranked_actions, no judge

**`run_judge()`** â€” called by `POST /debate/judge`:
...
- `GET /health` â€” reports load status for all three caches and the FAISS retrieval index (`has_pricing_cache`, `has_sentiment_cache`, `has_inventory_cache`, `has_retrieval_index`)
- `POST /pipeline` â€” fast retrieval + enrichment + baseline (no LLMs)
- `POST /debate/start` â€” advocate + critic round 1 (LLMs); takes pre-enriched candidates
- `POST /debate/continue` â€” one more advocate + critic revision round (LLMs)
- `POST /debate/judge` â€” runs the judge once on the final advocate + critic; returns `ranked_actions`
- `POST /orchestrate` â€” legacy all-in-one endpoint; kept for compatibility
- Lazy `get_pipeline()` â€” initialized once on first request using env vars:
...
- `inventory.risk_flag = true` â†’ **High**
- `sentiment.p_neg > 0.4` â†’ **High**
- `signals.total_returns > 3` or `sentiment.p_neg > 0.2` â†’ **Medium**
- Otherwise â†’ **Low**
...
### `recommended_price_change_pct` â€” not missing, it is a computed label
This column is **generated on demand** by `build_pricing_training_table.py` using a deterministic formula (elasticity-based grid search over Â±10% price change candidates). It was never a raw data column â€” it does not need to be "recovered" from anywhere.

To regenerate the pricing training table:
```bash
PYTHONPATH=copilot-v2/src python -m scripts.precompute.build_pricing_training_table \
...
PYTHONPATH=copilot-v2/src python -m scripts.precompute.build_pricing_cache \
...
PYTHONPATH=copilot-v2/src python -m scripts.precompute.build_sentiment_cache \
...
# Step 4: Build inventory cache (rule-based, no model needed â€” can run at any time)
PYTHONPATH=copilot-v2/src python -m scripts.precompute.build_inventory_cache \
  --snapshot-id 38710839ca6e1009 --artifacts-root copilot-v2/artifacts --write-json

# Step 5: Build per-owner retrieval indexes (requires sentence-transformers + faiss)
PYTHONPATH=copilot-v2/src python -m scripts.precompute.build_owner_indexes \
